...
const Shop = ({ categories, brands }: Props) => {
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
      ...
      if (selectedCategory) {
        conditions.push(`references(*[_type == "category" && slug.current == "${selectedCategory}"]._id)`);
      }
      if (selectedBrand) {
        conditions.push(`references(*[_type == "brand" && slug.current == "${selectedBrand}"]._id)`);
      }
      conditions.push(`price >= ${minPrice} && price <= ${maxPrice}`);

      const query = `*[_type == "product" && ${conditions.join(" && ")}] | order(name asc) {
        ...,
